...
The last 2 can be be selected in addition to the above codes - so for example, one ICD code could be both a Gastrointestinal and Technology Dependence:
...
Specific rules to format ICD Codes correclty:
...
* All codes in all categories employ "starts with substring" matching logic. Because of this, if a code to be evaluated starts with a code listed in one of the CCC categories, a match will be found. This means that if a bad ICD codes is provided (such as ICD9 code 0492,25042) PCCC would indicate a match for the Neuromuscular CCC.
* CCCs are matched in the order of the CCCs shown in the “Logic employed” section. Once a match is found, other categories are not evaluated with the exception of Technology Dependencies and Transplant CCCs.
* If there are changes in either ICD9 or ICD10, this library may require updating to continue functioning correctly.
