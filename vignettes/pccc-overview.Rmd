...
The `ccc` function is the workhorse here. Simply put, a user will provide ICD codes as strings and `ccc` will return CCC categories. All codes in all categories employ "starts with substring" matching logic, except as noted below. CCC codes for ICD-9-CM are matched on substrings and ICD 10 codes are matched on full codes, but all CCC categories use the same "starts with substring" matching logic.
  
Exceptions to this substring matching occur for selected codes. Some datasets may contain different degrees of specificty of ICD-9-CM codes, which can lead to issues with substring matching for certain codes. For example, consider a patient with _Congenital hereditary muscular dystrophy_. The least specific ICD-9-CM code for _Muscular dystropy_ is 359. The exact ICD-9-CM code specifying _Congenital hereditary muscular dystrophy_ is 3590. Even when describing the same exact patient, one dataset may contain the 359 code while another dataset may contain the 3590 code. If we use the substring logic above and match on 359, we would caputre the patient in both datasets. However, we would also capture all codes beginning with 359 in thedataset containing more specific ICD-9-CM codes, which could include non-CCC diagnoses like the code 3594, which specifies _Toxic myopathy_. If we use the substring logic and match on more specific codes like 3590, we would only capture the patient in the dataset with more specific ICD-9-CM codes. Thus, there is a tradeoff between sensitivity and specificity of detecting CCCs. We have provided an approach to address this problem by exact matching for less specific codes, e.g. the code 359 will match only if the dataset contains a 3-digit code 359, and substring matching for more specific codes, e.g. code 3590 will match any code beginning with 3590. This approach will improve the sensitivity of detecting CCCs in datasets with less specific codes, e.g. a physician who recorded the diagnosis of _Congenital hereditary muscular dystrophy_ as 359 and not 3590. At the same time, for datasets that contain more specific codes, this approach will reduce misclassifications, such as including 3594 as a CCC, so that we don't overestimate CCC counts. 
