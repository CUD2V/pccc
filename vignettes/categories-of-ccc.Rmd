...
showing the lists of codes for each category was published as a supplement to Feudtner 2014 and we
...
and `ccc` will return CCC categories. The ICD codes are matched exactly.  For example, ICD-9-CM
04.92 for subcategory "Devices"" under category "Neurologic or Neuromuscular"" will be
matched by the string 0492.  492 _will not_ match.  

The `data.frame` named `dat` below has three subjects, each of whom has a different, 
string for the ICD-9 code 04.92. The `ccc` function will only identify subject `A`
as having a "Neurologic or Neuromuscular" CCC.
...
Users of the `pccc` package will need to process the ICD-9
and ICD-10 codes they have so that the strings are formatted in the way that
the `pccc` package can recognize them.
